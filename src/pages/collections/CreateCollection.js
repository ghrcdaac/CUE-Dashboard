...
import {
  ...
  TablePagination, CircularProgress, TextField, Autocomplete
} from '@mui/material';
...
function CollectionFileBrowser() {
 ...
 const fetchCollections = useCallback(async (page) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/v1/collection/${selectedCollectionId}/files?ngroup_id=${ngroupId}&page=${filesPage + 1}&page_size=${filesPageSize}`,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );
      const data = await res.json();
      setFiles(data[0] || []);
